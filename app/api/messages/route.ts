...
const messageSchema = z.object({
  ...
  message: z
    .string()
    .min(1, "The message needs to be at least 1 character long"),
});
...
export async function POST(req: Request): Promise<void | Response> {
  ...
  try {
    ...
    const { data: message_data, error } = await supabase
      .from("messages")
      .insert([
        {
    ...
    if (error) return Response.server_error(error.message);
    ...
    const { error: conversation_update_error } = await supabase
      .from("conversations")
      .update({ last_message: newMessage.id })
    ...
    if (error instanceof z.ZodError) {
      ...
      return Response.server_error(errorMessage);
    }
    return Response.server_error("Unknown error");
  }
}
